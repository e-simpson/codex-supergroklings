...
- Keep role nickname candidates unique and limited to ASCII letters, digits, spaces, hyphens, and underscores. Do not use the model punctuation in `Grok 4.5`; an invalid nickname causes Codex to reject the entire custom-agent profile. Use the managed 30-name Grok-family pool (`Grok`, `Groki`, `Groku`, and the other bundled candidates) so concurrent workers remain recognizable and unique.
